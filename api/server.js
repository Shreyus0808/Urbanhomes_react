    ...
    db.query(q,[values],(err,data) => {
        if(err) return res.json(err)
          ...
          const notif_q = "INSERT INTO notifications (user_id,notification,created_at, updated_at) VALUES (?);";
                  ...
                  db.query(notif_q,[notif_values],(err,notif_data) => {
                      if(err) return res.json(err)
                      const userDetails_q = "INSERT INTO users_details (user_id, created_at, updated_at) VALUES (?, ?, ?);";
                      const userDetails_values = [
                          data.insertId,
                          createdAt,
                          updatedAt
                      ];

                      db.query(userDetails_q, userDetails_values, (err, userDetailsResult) => {
                          if (err) return res.json(err);
...
app.get('/profile/display/', (req,res) => {
  const q = req.query.user_id?"SELECT p.profile_pic,u.user_id,`user_type`, `full_name`, `email`, `phone`, `city`, `provience`,`about_us` FROM users p JOIN users_details u ON p.user_id = u.user_id WHERE u.user_id = ?":"SELECT * FROM users";
...
app.get('/UserProfile/Profile/:user_id', (req,res) => {
  const user_id = req.params.user_id;
  const q = "SELECT u.user_id, `profile_pic` ,`full_name`, `email`, `phone`, p.user_id, `city`, `provience`, `zip_code`, `company_name`, `about_us`, `experience` FROM users p JOIN users_details u ON p.user_id = u.user_id WHERE u.user_id = ?";
  db.query(q,[user_id],(err,data) => {
      if(err) return res.json(err)
      return res.json(data)
  })
